...
func monster_killed(_p_monster: Monster, _p_context: Battle.DefeatContext) -> void:
	initialize_panels()

func set_selected_item(p_item: int) -> void:
	if selected_item < items.size() and items[selected_item]:
		Global.current_battle.monsters[items[selected_item].monster_id].set_selected(false)
	Global.current_battle.monsters[items[p_item].monster_id].set_selected(true and focused)
...
func initialize_panels() -> void:
	if Global.current_battle == null:
		return
	if !Global.current_battle.killed_monster.is_connected(monster_killed):
		Global.current_battle.killed_monster.connect(monster_killed)
